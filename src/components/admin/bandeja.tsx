...
async function fetchAllPaginated<T>(path: string): Promise<T[]> {
  ...
  while (true) {
    ...
    const res = await api.get<PaginatedResult<T> | T[]>(url, { silentError: true });
        ...
        <div className="p-4">
          <Tabs value={tab} onValueChange={setTab}>
                  ...
                  <TableBody>
                    {inscripciones.map(({ preinscripcion: pre, prepagos }) => {
                      ...
                      return (
                        <TableRow key={pre.id}>
                          <TableCell>
                            {pre.nombreAlumno} {pre.aPaterno} {pre.aMaterno}
                          </TableCell>
                          ...
                          <TableCell>
                            <Sheet>
                              ...
                              >
                                <SheetHeader>
                                  <SheetTitle>
                                    Revisión de preinscripción #{pre.id}
                                  </SheetTitle>
                                </SheetHeader>
                                <div className="p-4 space-y-4 text-sm overflow-y-auto h-[calc(100vh-8rem)]">
                                  <div className="grid grid-cols-1 gap-2">
                                    <div>
                                      ...
                                      {pre.nombreAlumno} {pre.aPaterno}{" "}
                                      {pre.aMaterno}
                                    </div>
                                    <div className="grid grid-cols-2 gap-2">
                                      <div>
                                        <strong>DNI:</strong> {pre.nroDocumento}
                                      </div>
                                      <div>
                                        <strong>Sexo:</strong> {pre.sexo}
                                      </div>
                                      <div>
                                      ...
                                      <div>
                                        <strong>Email:</strong> {pre.email}
                                      </div>
                                      <div>
                                        <strong>Tel. Est.:</strong>{" "}
                                        {pre.telefonoEstudiante}
                                      </div>
                                      <div>
                                        <strong>Tel. Apod.:</strong>{" "}
                                        {pre.telefonoApoderado}
                                      </div>
                                      <div>
            ...
            <TabsContent value="pagos" className="mt-4 space-y-4">
              <div>
                <label className="text-sm" htmlFor="cicloFiltro">
                  Ciclo
                </label>
                <Select
                ...
                <TableBody>
                  {pagosFiltrados.map((item) => (
                    <TableRow key={item.pago.id}>
                      ...
                      <TableCell>
                        {item.alumno
                          ? `${item.alumno.nombreAlumno} ${item.alumno.aPaterno} ${item.alumno.aMaterno}`
                          : "-"}
                      </TableCell>
                      ...
                      <TableCell>
                        <Sheet>
                          ...
                          >
                            <SheetHeader>
                              <SheetTitle>
                                Revisión de pago #{item.pago.id}
                              </SheetTitle>
                            </SheetHeader>
                            <div className="p-4 space-y-4 text-sm overflow-y-auto h-[calc(100vh-8rem)]">
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  <strong>Fecha:</strong> {item.pago.fecha}
                                </div>
                                <div>
                                  <strong>Comprobante:</strong>{" "}
                                  {item.pago.nroVoucher}
                                </div>
                                <div>
                                  <strong>Monto:</strong> S/{" "}
                                  {item.pago.monto.toFixed(2)}
                                </div>
                                <div>
                                  <strong>Medio:</strong> {item.pago.medioPago}
                                </div>
                                <div>
                                  <strong>Ciclo:</strong>{" "}
                                  {item.inscripcion?.idCiclo
                                    ? cicloNombre[item.inscripcion.idCiclo] ??
                                      item.inscripcion.idCiclo
                                    : "-"}
                                </div>
                                <div>
                                  <strong>Documento:</strong>{" "}
                                  {item.alumno?.nroDocumento ?? "-"}
                                </div>
                              </div>
                              <div>
