...
export default function Page() {
      ...
      <header className="border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <Link to="/" className="flex items-center gap-2" aria-label="Inicio">
            <div className="size-8 rounded-full bg-primary text-primary-foreground grid place-items-center font-bold">CBT</div>
            <span className="font-semibold hidden sm:inline">Ciclo Básico Tecnológico</span>
          </Link>
          <nav className="flex items-center gap-2 sm:gap-4">
            <a href="#hero" className="text-sm hover:underline">Inicio</a>
            <a href="#nosotros" className="text-sm hover:underline">Nosotros</a>
            <a href="#ciclos" className="text-sm hover:underline">Ciclos</a>
            <a href="#grupos" className="text-sm hover:underline">Grupos</a>
            ...
            <Sheet>
              ...
              <SheetContent side="right">
                ...
                <div className="p-4 flex flex-col gap-3">
                  ...
                  <Button asChild size="lg" className="justify-start" variant="secondary">
  ...
  <div className="absolute inset-0 bg-linear-to-br from-[#7A1D1D]/80 via-[#7A1D1D]/60 to-transparent" aria-hidden />
        {/* Overlay respirando */}
        <motion.div
          aria-hidden
      ...
      <main className="flex-1">
        {/* Nosotros */}
        <section id="nosotros" className="max-w-6xl mx-auto px-4 py-12 md:py-16 space-y-8">
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground max-w-3xl mx-auto text-sm md:text-base">
              Nuestra plataforma educativa te brinda acceso a clases en vivo, materiales, simulacros y evaluaciones en línea, con un enfoque práctico y docente calificado.
            </p>
          </div>

          <div className="grid md:grid-cols-2 gap-4">
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} viewport={{ once: true, margin: "-80px" }}>
              <Card>
              <CardHeader>
                <CardTitle>Misión</CardTitle>
              </CardHeader>
              <CardContent className="text-sm text-muted-foreground">
                Formar estudiantes con visión tecnológica y valores, con metodologías innovadoras y acompañamiento permanente para su ingreso al IEST.
              </CardContent>
              </Card>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.1 }} viewport={{ once: true, margin: "-80px" }}>
              <Card>
              <CardHeader>
                <CardTitle>Visión</CardTitle>
              </CardHeader>
              <CardContent className="text-sm text-muted-foreground">
                Ser referentes en formación preuniversitaria y pre tecnológica, con excelencia académica y vinculación al entorno.
              </CardContent>
              </Card>
            </motion.div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[{
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.05 }} viewport={{ once: true, margin: "-80px" }}>
                <Card>
                <CardHeader>
          ...
          <div className="max-w-6xl mx-auto px-4 py-12 md:py-16 space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {[{
                ...
                <motion.div key={i} animate={{ y: [0, -4, 0] }} transition={{ duration: 6 + i, repeat: Infinity, ease: "easeInOut" }}>
                  <Card className="overflow-hidden">
                  <div className="flex">
                    <div className="bg-[#7A1D1D] text-white p-4 grid place-items-center min-w-24">
                      <div className="text-center">
                        <div className="text-2xl font-bold leading-none">{c.fecha.d}</div>
                        <div className="text-xs uppercase tracking-wide">{c.fecha.m}</div>
                      </div>
                    </div>
                    <div className="flex-1">
                      ...
                      <CardContent className="grid sm:grid-cols-3 gap-3 text-sm">
                        <div>
                          ...
                          <div className="text-muted-foreground">{c.costo}</div>
                        </div>
                        <div>
                          <div className="font-medium">Turno</div>
                          <div className="text-muted-foreground">{c.turno}</div>
                        </div>
                        <div>
                          <div className="font-medium">Exámenes</div>
                          <div className="text-muted-foreground">{c.examenes}</div>
                        </div>
      ...
      <footer className="border-t">
        <div className="max-w-6xl mx-auto px-4 py-8 text-sm text-muted-foreground flex flex-col sm:flex-row items-center justify-between gap-3">
          <div>© {new Date().getFullYear()} CBT - Todos los derechos reservados.</div>
          <div className="flex items-center gap-4">
            <a href="#nosotros" className="hover:underline">Nosotros</a>
            <a href="#ciclos" className="hover:underline">Ciclos</a>
            <a href="#grupos" className="hover:underline">Grupos</a>
          </div>
        </div>
      </footer>
    </div>
