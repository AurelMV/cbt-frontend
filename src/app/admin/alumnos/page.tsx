        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div className="md:col-span-2">
              <label className="text-sm" htmlFor="qAlumnos">
                Buscar (Nombre, DNI o Email)
              </label>
              <Input
                id="qAlumnos"
                value={q}
                ...
                placeholder="Ej. Perez 70123456"
              />
            </div>
            <div>
              <label className="text-sm" htmlFor="sexoFilter">
                Sexo
              </label>
              <Select
                value={sexo}
            ...
            <div>
              <label className="text-sm" htmlFor="colegioFilter">
                Colegio
              </label>
              <Input
                id="colegioFilter"
              ...
              <tbody>
                ...
                {items.map((a) => {
                  const nombreCompleto = [
                    ...
                    .filter(Boolean)
                    .join(" ");
                  const edadLabel = Number.isFinite(a.edad)
                    ? String(a.edad)
                    : formatAge(a.fechaNacimiento);
                  return (
                    <tr key={a.id} className="border-t">
                      ...
                      <td className="p-2">
                        {a.colegioNombre || a.idColegio || "—"}
                      </td>
                      <td className="p-2">{a.Direccion || "—"}</td>
                      <td className="p-2">
                        <EditAlumnoSheet alumno={a} onSaved={refetch} />
                      </td>
                    </tr>
                  );
                })}
          ...
          <div className="flex items-center justify-between pt-2">
            <div className="text-sm text-muted-foreground">
              {data
                ? `Total: ${data.total} (pág. ${data.page + 1} de ${
                    data.pages
                  })`
                : ""}
            </div>
...
function EditAlumnoSheet({
  alumno,
  onSaved,
}: Readonly<{
  alumno: import("@/services/alumnos").AlumnoListItem;
  onSaved: () => void;
}>) {
  const updateMutation = useUpdateAlumno();
  const form = useForm<import("@/services/alumnos").AlumnoUpdate>({
    resolver: zodResolver(alumnoSchema) as Resolver<
      import("@/services/alumnos").AlumnoUpdate
    >,
    defaultValues: {
        ...
        <div className="p-4">
          <form onSubmit={onSubmit} className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field>
                <FieldLabel htmlFor={`al-nombre-${alumno.id}`}>
                  Nombres
                </FieldLabel>
                <FieldContent>
                  <Input
                    id={`al-nombre-${alumno.id}`}
              ...
              <Field>
                <FieldLabel htmlFor={`al-paterno-${alumno.id}`}>
                  A. paterno
                </FieldLabel>
                <FieldContent>
                  <Input
                    id={`al-paterno-${alumno.id}`}
              ...
              <Field>
                <FieldLabel htmlFor={`al-materno-${alumno.id}`}>
                  A. materno
                </FieldLabel>
                <FieldContent>
                  <Input
                    id={`al-materno-${alumno.id}`}
              ...
              <Field>
                <FieldLabel htmlFor={`al-fecha-${alumno.id}`}>
                  Fecha nac.
                </FieldLabel>
                <FieldContent>
                  <Input
                    id={`al-fecha-${alumno.id}`}
                    type="date"
              ...
              <Field>
                ...
                <FieldContent>
                  <Input
                    id={`al-ano-${alumno.id}`}
                    type="number"
                    {...form.register("anoCulminado", { valueAsNumber: true })}
                  />
                  <FieldError errors={[form.formState.errors.anoCulminado]} />
                </FieldContent>
              </Field>
              <Field>
                <FieldLabel htmlFor={`al-dir-${alumno.id}`}>
                  Dirección
                </FieldLabel>
                <FieldContent>
                  <Input
              ...
              <Field>
                ...
                <FieldContent>
                  <Input
                    id={`al-col-${alumno.id}`}
                    type="number"
                    {...form.register("idColegio", { valueAsNumber: true })}
                  />
                  <FieldError errors={[form.formState.errors.idColegio]} />
                </FieldContent>
              </Field>
              <Field>
                <FieldLabel htmlFor={`al-telE-${alumno.id}`}>
                  Tel. Estudiante
                </FieldLabel>
                <FieldContent>
                  <Input
                    id={`al-telE-${alumno.id}`}
                    {...form.register("telefonoEstudiante")}
                  />
                  <FieldError
                    errors={[form.formState.errors.telefonoEstudiante]}
                  />
                </FieldContent>
              </Field>
              <Field>
                <FieldLabel htmlFor={`al-telA-${alumno.id}`}>
                  Tel. Apoderado
                </FieldLabel>
                <FieldContent>
                  <Input
                    id={`al-telA-${alumno.id}`}
                    {...form.register("telefonoApoderado")}
                  />
                  <FieldError
                    errors={[form.formState.errors.telefonoApoderado]}
                  />
                </FieldContent>
              </Field>
            </div>
