...
export default function Page() {
  ...
  useEffect(() => { setPage(1) }, [ciclo, estado, tipoPago])
  ...
  const enriched = useMemo(() => {
    return rows.map((pago) => {
      ...
      const apellidos = [pago.aPaterno, pago.aMaterno].filter(Boolean).join(" ") || "—"
  ...
  const filtrados = useMemo(() => {
    ...
    return enriched.filter((r) =>
      [r.codigoPago, r.nombreEst, r.apellidos, r.fecha].some((value) => value.toLowerCase().includes(term))
    )
  }, [enriched, q])
    ...
    <div className="p-4 md:p-6 space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <div>
              <label className="text-sm" htmlFor="cicloPagos">Ciclo</label>
              <Select value={ciclo || "all"} onValueChange={(v) => setCiclo(v === "all" ? "" : v)}>
                <SelectTrigger id="cicloPagos">
                  <SelectValue placeholder={ciclosQ.isLoading ? "Cargando…" : "Todos"} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              <label className="text-sm" htmlFor="estadoPagos">Estado</label>
            ...
            <div>
              <label className="text-sm" htmlFor="tipoPagos">Tipo de pago</label>
              <Select value={tipoPago || "all"} onValueChange={(v) => setTipoPago(v === "all" ? "" : v)}>
                <SelectTrigger id="tipoPagos">
                  <SelectValue placeholder={tipoOptions.length ? "Todos" : "Sin datos"} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">Todos</SelectItem>
          ...
          <Table>
            ...
            <TableBody>
              {isError && (
                <TableRow><TableCell colSpan={9} className="text-red-600">No se pudieron cargar pagos.</TableCell></TableRow>
              )}
              {isLoading && (
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" disabled={page >= pages} onClick={() => setPage((p) => Math.min(pages || 1, p + 1))}>Siguiente</Button>
        </div>
      </div>
    </div>
  )
...
function EditPagoSheet({ pago, onSaved }: Readonly<{ pago: PagoRead; onSaved: () => void }>) {
